...
export type ApiDependencies = { jwtSecret: string; findUserByEmail: (email: string) => Promise<User | null>; findUserById: (id: number) => Promise<User | null>; resolvePartnerAccessScope: (user: Pick<User, "id" | "role">) => Promise<PartnerAccessScope>; listUsers: () => Promise<ManagedUser[]>; createManagedUser: (email: string, password: string, role: UserRole) => Promise<ManagedUser>; updateManagedUser: (id: number, update: UserAdminUpdate) => Promise<ManagedUser | null>; updateManagedUserPassword: (id: number, password: string) => Promise<boolean>; getPartnerNames: (scope: PartnerAccessScope) => Promise<string[]>; getAllPromos: (scope: PartnerAccessScope) => Promise<PromoRecord[]>; getPromoById: (id: string, scope: PartnerAccessScope) => Promise<PromoRecord | null>; getPendingReports: (scope: PartnerAccessScope) => Promise<PendingReportRecord[]>; setReportState: (id: string, received: boolean, scope: PartnerAccessScope) => Promise<ReportRecord>; deletePromo: (id: string) => Promise<boolean>; removePromoPartner: (promoId: string, partnerId: string) => Promise<boolean>; now: () => Date };
const defaults: Omit<ApiDependencies, "jwtSecret"> = { findUserByEmail, findUserById, resolvePartnerAccessScope, listUsers, createManagedUser, updateManagedUser, updateManagedUserPassword, getPartnerNames: getWorkspacePartnerNames, getAllPromos: getWorkspacePromos, getPromoById: getWorkspacePromoById, getPendingReports: getWorkspacePendingReports, setReportState: setWorkspaceReportState, deletePromo, removePromoPartner, now: () => new Date() };
