...
function deps(overrides: Partial<ApiDependencies> = {}): Partial<ApiDependencies> { return { jwtSecret: secret, findUserByEmail: vi.fn().mockResolvedValue(user), findUserById: vi.fn().mockResolvedValue(user), getAllPromos: vi.fn().mockResolvedValue([promoRecord]), getPromoById: vi.fn().mockResolvedValue(promoRecord), getPendingReports: vi.fn().mockResolvedValue([pendingRecord]), getPromoPartnerById: vi.fn().mockResolvedValue(relation), markReportReceived: vi.fn().mockResolvedValue({ ...relation, reportReceived: true, reportReceivedAt: now }), markReportPending: vi.fn().mockResolvedValue(relation), deletePromo: vi.fn().mockResolvedValue(true), removePromoPartner: vi.fn().mockResolvedValue(true), now: () => now, ...overrides }; }
...
describe("Promo Tracker API", () => {
  ...
  it("PATCH report true calls markReportReceived", async () => { const mark = vi.fn().mockResolvedValue({ ...relation, reportReceived: true, reportReceivedAt: now }); const response = await request(createApi(deps({ markReportReceived: mark }))).patch("/api/promo-partners/relation-1/report").set("Cookie", cookie).send({ received: true }); expect(response.status).toBe(200); expect(mark).toHaveBeenCalledWith("relation-1"); });
  it("PATCH report false calls markReportPending", async () => { const mark = vi.fn().mockResolvedValue(relation); const response = await request(createApi(deps({ markReportPending: mark }))).patch("/api/promo-partners/relation-1/report").set("Cookie", cookie).send({ received: false }); expect(response.status).toBe(200); expect(mark).toHaveBeenCalledWith("relation-1"); });
  ...
  it("PATCH report returns 404 for an unknown relation", async () => expect((await request(createApi(deps({ getPromoPartnerById: vi.fn().mockResolvedValue(null) }))).patch("/api/promo-partners/missing/report").set("Cookie", cookie).send({ received: true })).status).toBe(404));
});
