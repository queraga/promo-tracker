...
function deps(user: User | null, overrides: Partial<ApiDependencies> = {}): Partial<ApiDependencies> { return { jwtSecret: secret, findUserByEmail: vi.fn().mockImplementation(async (email) => user?.email === email.trim().toLowerCase() ? user : null), findUserById: vi.fn().mockImplementation(async (id) => user?.id === id ? user : null), getAllPromos: vi.fn().mockResolvedValue([promoRecord]), getPromoById: vi.fn().mockResolvedValue(promoRecord), getPendingReports: vi.fn().mockResolvedValue([]), setReportState: vi.fn().mockResolvedValue({ ...relation, reportReceived: true }), deletePromo: vi.fn().mockResolvedValue(true), removePromoPartner: vi.fn().mockResolvedValue(true), now: () => createdAt, ...overrides }; }
