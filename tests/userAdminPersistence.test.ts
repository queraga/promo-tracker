...
describe("user administration persistence", () => {
  it("lists users without password hashes", async () => {
    await createManagedUser("z@example.com", "strong-password", "USER");
  ...
  it("changes role and active state", async () => {
    ...
    expect(await updateManagedUser(admin.id, { role: "USER", isActive: false })).toMatchObject({ role: "USER", isActive: false });
  });
  ...
  it("updates the bcrypt password hash", async () => {
    const user = await createManagedUser("user@example.com", "old-password", "USER");
  ...
  it("returns null/false for an unknown user", async () => {
    await expect(updateManagedUser(999, { role: "USER" })).resolves.toBeNull();
