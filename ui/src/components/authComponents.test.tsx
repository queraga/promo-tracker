...
describe("authentication components", () => {
  ...
  it("renders PLM promo and report details as intentionally read-only", () => { const html = drawer("PLM"); expect(html).toContain("Rozetka"); expect(html).toContain("Очікується звіт"); expect(html).not.toContain("Позначити отриманим"); expect(html).not.toContain("Додати партнерів"); expect(html).not.toContain("Видалити промо"); expect(html).not.toContain("Видалити зв’язок"); });
  it("renders user provisioning, supported role labels, and password controls for SUPERUSER administration", () => { const html = renderToStaticMarkup(<UserManagement currentUser={user("SUPERUSER")} onCurrentUserChange={vi.fn()} onError={vi.fn()} />); expect(html).toContain("Новий користувач"); expect(html).toContain("Тимчасовий пароль"); expect(html).toContain('value="KAM"'); expect(html).toContain('value="PLM"'); expect(html).toContain('value="SUPERUSER"'); expect(html).not.toContain('value="USER"'); });
});
