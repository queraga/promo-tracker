...
describe("admin UI logic", () => {
  ...
  it("keeps PLM read-only while preserving KAM and SUPERUSER business actions", () => {
    ...
    expect(canExpandPromoPartners(user("PLM"))).toBe(false);
    expect(canMutateReports(user("KAM"))).toBe(true);
    expect(canExpandPromoPartners(user("KAM"))).toBe(true);
    expect(canMutateReports(user("SUPERUSER"))).toBe(true);
  });
